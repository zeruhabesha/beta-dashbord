...
# 1. Restore/Create Visualizations (Metric, Pie, Histogram)
$viz1 = @{
    attributes = @{
        ...
        description           = ""
        version               = 1
        kibanaSavedObjectMeta = @{ searchSourceJSON = '{"index":"13c1840","query":{"query":"","language":"kuery"},"filter":[]}' }
    }
}
$viz2 = @{
    attributes = @{
...
Write-Host "Restoring Visualizations..."
...
# 2. Create Dedicated SIEM Dashboard (charts + tables)
...
Write-Host "Creating SIEM Dashboard..."
...
Write-Host "Done. Dashboard ID: dashboard-siem-main"
